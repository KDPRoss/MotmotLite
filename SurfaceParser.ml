...
let rec knd : Syntax.knd lazyParse =
  ( let core () =
        ( let karr = ( cache kndS  <== txt "->" <=> cache knd >>> fun kk' -> Syntax.KArr kk' ) in
        karr ||| cache kndS  ) in
  lazy ( core () ) )

and kndS  : Syntax.knd lazyParse =
  ( let core () =
        ( let kstar = ( txt "*" >>> const Syntax.KStar )
            in let kbkt = ( parens ( cache knd ) ) in
        kstar ||| kbkt ) in
  lazy ( core () ) )
...
let rec typ : SurfaceSyntax.typ lazyParse =
  ( let core () =
        ( let tarr = ( cache typH  <== txt "->" <=> cache typ >>> fun tt' -> SurfaceSyntax.TArr tt' )
            in let tabs = ( parens ( commaSepList1    varP  <== txt ":" <=> cache knd ) <== txt "=>" <=> cache typ >>> fun ( ( xs, k ) , t ) -> SurfaceSyntax.TAbs ( xs, k, t ) ) in
        tarr ||| tabs ||| cache typH  ) in
  lazy ( core () ) )

and typH  : SurfaceSyntax.typ lazyParse =
  ( let core () =
        ( let tapp = ( many1Spaces   ( cache typS  ) >>> fold1 ~f: ( fun t t' -> SurfaceSyntax.TApp ( t, t' ) ) ) in
        tapp ) in
  lazy ( core () ) )
...
let parserFuncs  : SurfaceSyntax.pat lazyParse * SurfaceSyntax.pat lazyParse * SurfaceSyntax.exp lazyParse * SurfaceSyntax.exp lazyParse * SurfaceSyntax.exp lazyParse * SurfaceSyntax.exp lazyParse =
  ( let rec patExt  : SurfaceSyntax.pat lazyParse =
      ...
      and patE  : SurfaceSyntax.pat lazyParse =
        ( let core () =
              ( let pcons = ( cache patH  <== txt "::" <=> cache patE  >>> fun ( p, p' ) -> SurfaceSyntax.PListCons ( p, p' ) ) in
              pcons ||| cache patH  ) in
        lazy ( core () ) )
      ...
      and patS  : SurfaceSyntax.pat lazyParse =
        ( let core () =
              ( let pany = ( parens ( txt "_" ==> txt ":" ==> cache typ ) >>> fun t -> SurfaceSyntax.PAny t )
                  ...
                  in let ppred = ( txt "{" =*> cache exp <*= txt "}" >>> fun e -> SurfaceSyntax.PPred e )
                  in let pexp = ( txt "`" =*> cache expS  >>> fun e -> SurfaceSyntax.PEq e )
      ...
      and exp : SurfaceSyntax.exp lazyParse =
        ( let core () =
              ( let ann = ( cache expB  <== txt ":" <=> cache typ >>> fun et -> SurfaceSyntax.EAnn et ) in
              ann ||| cache expB  ) in
        lazy ( core () ) )
      and etabs = ( lazy ( parens ( varP  <== txt ":" <=> cache knd ) <== txt "=>" <=> cache expB  >>> fun ( ( x, k ) , e ) -> SurfaceSyntax.ETAbs ( [ x ] , k, e ) ) )
      and eabs = ( lazy ( repsep1 ( cache patExt  <== just () ) ( txt "|" <== just () ) <== txt "~" <!> cache expB  >>> fun pse -> SurfaceSyntax.EAbs pse ) )
      ...
      and expH  : SurfaceSyntax.exp lazyParse =
        ( let core () =
              ( let handleApp  ( es : SurfaceSyntax.exp list ) : SurfaceSyntax.exp parse =
                    ( match ( es &>
                           ResolveApp.appTreesOfExps    @>
                           ResolveApp.resolveApps  ) with
                    | Some t -> ( let e = ( ResolveApp.expOfAppTree    t ) in
                                just e )
                    | None -> ( fail "Invalid application." ) )
                  in let eapp = ( many1Spaces   ( cache expT  ) >>= handleApp  )
                  in let tapp = ( cache expT  <*= txt "{" <!> commaSepList1    ( cache typ ) <!= txt "}" >>> fun ets -> SurfaceSyntax.ETAppMany ets )
                  in let lsec = ( txt "{" =!> cache typ <!= txt "}" <!> cache expT  <=> cache expT  >>> fun ( ( t, x ) , e ) -> SurfaceSyntax.ELSec ( t, x, e ) )
                  in let rsec = ( cache expT  <=> cache expT  <!> ( txt "{" =!> cache typ <!= txt "}" ) >>> fun ( ( e, x ) , t ) -> SurfaceSyntax.ERSec ( e, x, t ) )
                  in let limp = ( txt "_" =!> cache expT  <=> cache expT  >>> fun ( x, e ) -> SurfaceSyntax.ELSecImp ( x, e ) )
                  in let rimp = ( cache expT  <=> cache expT  <!= txt "_" >>> fun ( e, x ) -> SurfaceSyntax.ERSecImp ( e, x ) )
                  in let lscb = ( txt "{" =!> cache typ <!= txt "}" <!> many1Spaces   ( cache expT  ) >>= fun ( t, es ) ->
                         ( match ( es ) with
                         | SurfaceSyntax.EVar x :: es -> ( handleApp  es >>= fun e ->
                  ...
                  in let limb = ( txt "_" =!> many1Spaces   ( cache expT  ) >>=
                         function
                         | SurfaceSyntax.EVar x :: es -> ( handleApp  es >>= fun e ->
              ...
              rimb ||| limb ||| rscb ||| lscb ||| limp ||| rimp ||| rsec ||| lsec ||| tapp ||| eapp ) in
        lazy ( core () ) )
