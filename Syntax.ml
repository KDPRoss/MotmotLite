...
type knd = KStar | KArr of (knd * knd)
...
and natvFunc = Natv1 of (exp -> exp) | Natv2 of (exp -> exp -> exp)

exception UserFailure of (string * exp)

let natNumArgs : natvFunc -> int = function Natv1 _ -> 1 | Natv2 _ -> 2

exception EqIncomparableEsc of (exp * exp)

let rec eqableQ : exp -> bool = function
  | ETup es | ECVal (_, es) | EList es -> List.for_all ~f:eqableQ es
  | ENum _ -> true
  | EMap m ->
      let ksQ = m &> PolyMap.keys @> List.for_all ~f:eqableQ in
      ksQ && PolyMap.for_all ~f:eqableQ m
  | _ -> false

let rec eqCoreBool (throwForFunc : bool) (v1 : exp) (v2 : exp) : bool =
  let eqCoreBool = eqCoreBool throwForFunc in
  let _ =
    if throwForFunc && not (eqableQ v1 && eqableQ v2) then
      raise (EqIncomparableEsc (v1, v2))
  in
  match (v1, v2) with
  | EMap m1, EMap m2 -> Core.Map.equal eqCoreBool m1 m2
  | ENum n1, ENum n2 -> n1 = n2
  | ECVal (c1, es1), ECVal (c2, es2) ->
      if c1 <> c2 then false else eqCoreBool (ETup es1) (ETup es2)
  | ETup ts1, ETup ts2 ->
      if List.length ts1 <> List.length ts2 then false
      else List.zip_exn ts1 ts2 &> List.for_all ~f:(uncurry eqCoreBool)
  | EList vs1, EList vs2 -> (
      let open Core.List.Or_unequal_lengths in
...
let patBoundVarsWithTypes : pat -> (string * typ) list =
  let rec patBoundVars : pat -> (string * typ) PolySet.t =
    ...
    | PCVal (_, _, ps) | PTup ps ->
        ps
        &> List.map ~f:patBoundVars @> List.fold ~init:PolySet.empty ~f:( ++ )
    | PConj (p, p') -> patBoundVars p ++ patBoundVars p'
    | _ -> PolySet.empty
  in
  patBoundVars @> PolySet.to_list
...
let rec patBoundVars : pat -> StringSet.t =
  ...
  | PCVal (_, _, ps) | PTup ps ->
      ps
      &> List.map ~f:patBoundVars @> List.fold ~init:StringSet.empty ~f:( ++ )
  | PConj (p, p') -> patBoundVars p ++ patBoundVars p'
  | _ -> StringSet.empty
...
let rec patFreeVars : pat -> StringSet.t =
  ...
  function
  | PCVal (_, _, ps) | PTup ps ->
      ps &> List.map ~f:patFreeVars @> List.fold ~init:StringSet.empty ~f:( ++ )
  | PConj (p1, p2) -> patFreeVars p1 ++ patFreeVars p2
  | PPred e | PWhen (e, _) -> expFreeVars e
  | _ -> StringSet.empty
...
and expFreeVars : exp -> StringSet.t =
  ...
  | ETAbs (_, _, e) | ETApp (e, _) -> expFreeVars e
  | EAbs (ps, e) ->
      let rec loop bound free =
       (function
       | [] -> (bound, free)
       | p :: ps ->
           let fs = patFreeVars p // bound in
           let bs = patBoundVars p in
           loop (bound ++ bs) (free ++ fs) ps)
      in
  ...
  | ETup es | EFcmp es | ECVal (_, es) | EList es ->
      es &> List.map ~f:expFreeVars @> List.fold ~init:StringSet.empty ~f:( ++ )
  | ENum _ -> StringSet.empty
  | ELet (bs, e) ->
      let bound =
        bs
        &> List.map ~f:(fst @> patBoundVars)
           @> List.fold ~init:StringSet.empty ~f:( ++ )
      in
      let free =
        e :: List.map ~f:snd bs
        &> List.map ~f:expFreeVars @> List.fold ~init:StringSet.empty ~f:( ++ )
      in
      free // bound
  | ECls _ | EDely _ | ELazy _ | ENatF _ | EMap _ -> StringSet.empty
