...
let numP : Q.t parse =
  let postProcess s =
    ...
    | [ w; f ] ->
        let d = Q.of_string ("1" -- String.make (String.length f) '0') in
        let f' = Q.(of_string f / d) in
        w &> Q.of_string @> Q.( + ) f' @> just
    | [ w ] -> w &> Q.of_string @> just
    | _ -> fail "Bad number."
  in
  let oldStyle = regexp (Str.regexp "[0-9]+\\([.][0-9]+\\)?") >>= postProcess in
  let postProcess = function Some _, n -> Q.neg n | None, n -> n in
  maybe (txt "-") <*> oldStyle >>> postProcess

let typPrim : Syntax.prim -> Syntax.typ = function Num _ -> TCVal ("Num", [])
...
let primKindEnv parseKnd =
  let assumps = [ ("Num", parseKnd "*") ] in
  match PolyMap.of_alist assumps with
  | `Duplicate_key k ->
      failwith
        ("Duplicate key for `" -- k
       -- "` in `primKindEnv`; this is an implementation error.")
  | `Ok m -> Env.ofMap m
