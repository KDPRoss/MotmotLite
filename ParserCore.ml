...
type 'a result = Success of 'a * int * int
               | Failure of int
...
let setIx  : int -> unit m =
  ( fun ix ( _, err ) ->
    ( Success ( () , ix, err ) ) )

let fail : 'a m =
  ( fun ( err1, err2 ) ->
    ( Failure ( max err1 err2 ) ) )

let makeParser  core = ( { cachedResults  = None ;
                         parser = core ;
                       } )
...
let chr c =
  ( let core = ( fun { len ; str ; _ } ->
               ( getIx  >>= fun ix ->
               ( if ( ix < len && str. [ ix ] = c )
                  then ( setIx  ( ix + 1 ) >>= fun _ ->
                       ( return str. [ ix ] ) )
                  else ( fail ) ) ) ) in
  makeParser  core )
...
let txtGuarded  v ~isnt =
  ( let vLen  = ( String.length v )
      in let core = ( fun { len ; str ; _ } ->
               ( getIx  >>= fun ix ->
               ( let rec loop i = ( if ( i = vLen  && ( i = len || not ( isnt str. [ ix ] ) ) )
                                     then ( setIx  ( ix + vLen  ) >>= fun _ ->
                                          ( return v ) )
                                     else ( if ( i < vLen  && ix + i < len && v. [ i ] = str. [ ix + i ] )
                                             then ( loop ( i + 1 ) )
                                             else ( fail ) ) ) in
               loop 0 ) ) ) in
  makeParser  core )
...
let regexp reg =
  ( let core = ( fun { str ; _ } ->
               ( getSt  >>= fun ( ix, err ) ->
               ( if ( Str.string_match reg str ix )
                  then ( let v = ( Str.matched_string str ) in
                       setIx  ( Str.match_end () ) >>= fun _ ->
                       ( return v ) )
                  else ( setIx  err >>= fun _ ->
                       ( fail ) ) ) ) ) in
  makeParser  core )
...
let cache lz =
  ( let core = ( fun ( { cleanup ; len ; _ } as src ) ( ix, err ) ->
               ( if ( ix >= len )
                  then ( failure err ix )
                  else ( let p = ( Lazy.force lz )
                           in let arr = ( match ( p.cachedResults  ) with
                                 | None -> ( let arr = ( Array.make len None )
                                                   in let _ = ( p.cachedResults  <- Some arr )
                                                   in let _ = ( src.cleanup <- ( fun () -> p.cachedResults  <- None ) :: cleanup ) in
                                               arr )
                                 | Some arr -> ( arr ) )
                           in let res = ( match ( arr. ( ix ) ) with
                                 | Some ( Success ( v, ix, err ) ) -> ( Success ( v, ix, err ) )
                                 | Some fl -> ( fl )
                                 | None -> ( let res = ( p.parser src ( ix, 0 ) )
                                                                      in let _ = ( arr. ( ix ) <- Some res ) in
                                                                  res ) ) in
                       match ( res ) with
                       | Success ( v, ix, err1 ) -> ( Success ( v, ix, max err err1 ) )
                       | Failure err1 -> ( failure err err1 ) ) ) ) in
  makeParser  core )

let ( >>= ) pa fb =
  ( let core = ( fun src ->
               ( pa.parser src >>= fun res ->
               ( ( fb res ) .parser src ) ) ) in
  makeParser  core )

let rep p =
  ( let core = ( fun src ->
               ( let rec loop vs ( ix, err ) = ( match ( p.parser src ( ix, err ) ) with
                                           | Success ( v, ix, err ) -> ( loop ( v :: vs ) ( ix, err ) )
                                           | Failure err -> ( Success ( List.rev vs, ix, err ) ) ) in
               loop [] ) ) in
  makeParser  core )
...
let eof =
  ( let core = ( fun { len ; _ } ( ix, err ) ->
               ( if ( ix = len )
                  then ( Success ( () , ix, err ) )
                  else ( failure err ix ) ) ) in
  makeParser  core )

let identOf  fst rest =
  ( let core = ( fun ( { len ; str ; _ } ) ( ixStart,  err ) ->
               ( let not = ( Stdlib.not ) in
               let rec loop ix = ( if ( ix = len )
                                    then ( let tok = ( String.sub ~pos:ixStart  ~len: ( ix - ixStart  ) str ) in
                                         Success ( tok, ix, err ) )
                                    else ( if ( not ( rest str. [ ix ] ) )
                                            then ( let tok = ( String.sub ~pos:ixStart  ~len: ( ix - ixStart  ) str ) in
                                                 Success ( tok, ix, err ) )
                                            else ( loop ( ix + 1 ) ) ) ) in
               if ( ixStart  < len )
                  then ( if ( fst str. [ ixStart  ] )
                          then ( loop ixStart  )
                          else ( Failure err ) )
                  else ( Failure err ) ) ) in
  makeParser  core )
